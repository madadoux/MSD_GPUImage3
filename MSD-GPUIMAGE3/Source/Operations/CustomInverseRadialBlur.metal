...
typedef struct
{
    ...
    float centerX;
    float centerY;
} InverseRadialBlurUniform;

fragment half4 CustomInverseRadialBlurFragment(SingleInputVertexIO fragmentInput [[stage_in]],
                                texture2d<half> inputTexture [[texture(0)]],
                                texture2d<half> inputTexture2 [[texture(1)]],
                                constant InverseRadialBlurUniform& uniform [[ buffer(1) ]])
{
    ...
    {
        float ratio = (outerElp - radius) / (outerElp-1.0);
